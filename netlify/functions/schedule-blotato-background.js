// One-time Netlify BACKGROUND function — call this ONCE to schedule all 30 days of IsItGoneDown
// posts in Blotato at 9 AM UTC per day (April 7 – May 6 2026).
// Background functions return 202 immediately and run for up to 15 min — no browser timeout.
// Trigger: /.netlify/functions/schedule-blotato-background?confirm=yes
// Monitor progress: Netlify dashboard → Functions → schedule-blotato-background → logs

const https = require('https');

const BLOTATO_API_KEY = 'blt_GV1PlBxKT6OSshia7334Tm4yen/n0q3hcQzJF/6obfU=';
const CAMPAIGN_START_DATE = '2026-04-07';
const POST_HOUR_UTC = 9; // 9:00 AM UTC
...
exports.handler = async (event) => {
  // Background functions return 202 immediately — this handler runs async for up to 15 min.
  ...
  if (confirm !== 'yes') {
    console.log('Missing ?confirm=yes — aborting.');
    return;
  }
  ...
  for (let dayIndex = 0; dayIndex < CONTENT.length; dayIndex++) {
    ...
    for (const account of ACCOUNTS) {
      ...
      try {
        ...
        console.log(`Day ${dayNumber} ${account.platformKey}: ${result.status} — ${scheduledTime}${success ? '' : ' ERROR: ' + JSON.stringify(result.body)}`);
      } catch (err) {
        errorCount++;
        console.error(`Day ${dayNumber} ${account.platformKey} EXCEPTION:`, err.message);
      }
  ...
  console.log(`Schedule run complete. ${successCount} posts queued, ${errorCount} errors.`);
};
